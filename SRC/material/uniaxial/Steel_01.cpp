...
// $Source: /usr/local/cvs/OpenSees/SRC/material/uniaxial/Steel_01.cpp,v $
...
OPS_Steel_01()
{
  ...
  if (OPS_GetIntInput(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial Steel_01 tag" << endln;
    return 0;
  }
  ...
  if (numData != 3 && numData != 7) {
    opserr << "Invalid #args, want: uniaxialMaterial Steel_01 " << iData[0] << " fy? E? b? <a1? a2? a3? a4?>>" << endln;
    return 0;
  }
  ...
  theMaterial = new Steel_01(iData[0], dData[0], dData[1], 
			    dData[2], dData[3], dData[4], 
			    dData[5], dData[6]);

  
  if (theMaterial == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type Steel_01 Material" << endln;
    return 0;
  }
...
(int tag, double FY, double E, double B,
 double A1, double A2, double A3, double A4):
   UniaxialMaterial(tag,MAT_TAG_Steel_01),
...
Steel_01::Steel_01():UniaxialMaterial(0,MAT_TAG_Steel_01),
...
int Steel_01::setTrialStrain (double strain, double strainRate)
{
...
int Steel_01::setTrial (double strain, double &stress, double &tangent, double strainRate)
{
   // Reset history variables to last converged state
   TminStrain = CminStrain;
   TmaxStrain = CmaxStrain;
...
void Steel_01::determineTrialState (double dStrain)
{
...
void Steel_01::detectLoadReversal (double dStrain)
{
   // Determine initial loading condition
   if (Tloading == 0 && dStrain != 0.0)
   {
...
double Steel_01::getStrain ()
{
   return Tstrain;
}

double Steel_01::getStress ()
{
   return Tstress;
}

double Steel_01::getTangent ()
{
   return Ttangent;
}

int Steel_01::commitState ()
{
   // History variables
...
int Steel_01::revertToLastCommit ()
{
...
UniaxialMaterial* Steel_01::getCopy ()
{
   Steel_01* theCopy = new Steel_01(this->getTag(), fy, E0, b,
				  a1, a2, a3, a4);
...
int Steel_01::sendSelf (int commitTag, Channel& theChannel)
{
   int res = 0;
   ...
   if (theChannel.sendVector(dbTag, commitTag, data) < 0) {
     opserr << "Steel_01::sendSelf() - failed to send data" << endln;
     return -1;
   }

   if (SHVs != 0) {
     if (theChannel.sendMatrix(dbTag, commitTag, *SHVs) < 0) {
       opserr << "Steel_01::sendSelf() - failed to send SHVs matrix" << endln;
       return -2;
     }
   }
...
int Steel_01::recvSelf (int commitTag, Channel& theChannel,
                                FEM_ObjectBroker& theBroker)
{
  ...
  static Vector data(19);
   ...
   if (theChannel.recvVector(this->getDbTag(), commitTag, data) < 0) {
     opserr << "Steel_01::recvSelf() - failed to receive data" << endln;
      this->setTag(0);
      return -1;
   }
   ...
   if (noCols > 0) {
     ...
     if (SHVs == 0) {
       opserr << "Steel_01::recvSelf() - failed to allocate SHVs matrix" << endln;
       return -2;
     }
     
     if (theChannel.recvMatrix(dbTag, commitTag, *SHVs) < 0) {
       opserr << "Steel_01::recvSelf() - failed to receive SHVs matrix" << endln;
       return -3;
     }
   }      
   //SAJalali
...
void Steel_01::Print (OPS_Stream& s, int flag)
{
  if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {    
    s << "Steel_01 tag: " << this->getTag() << endln;
    s << "  fy: " << fy << " ";
    s << "  E0: " << E0 << " ";
    s << "   b: " << b << " ";
    s << "  a1: " << a1 << " ";
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
Steel_01::setParameter(const char **argv, int argc, Parameter &param)
{
...
int
Steel_01::updateParameter(int parameterID, Information &info)
{
	switch (parameterID) {
	case -1:
		return -1;
...
int
Steel_01::activateParameter(int passedParameterID)
{
...
double
Steel_01::getStressSensitivity(int gradIndex, bool conditional)
{
...
double
Steel_01::getInitialTangentSensitivity(int gradIndex)
{
...
int
Steel_01::commitSensitivity(double TstrainSensitivity, int gradIndex, int numGrads)
{
	if (SHVs == 0) {

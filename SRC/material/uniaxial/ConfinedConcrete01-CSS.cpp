...
//helper functions:
double confAlongCol(double semiLength, double phis, double S, double phiLon) {
...
void setupAttardSetunge(double fpc, double stRatio, double Ec, double aggrType,
	 double concrType, double& epsc,	 double& epsic, double& fic, double& ft, double& fpl,
	 double& alpha, double& Eti, double& Ec0) {
...
void trReinfModSqSec(double syh, double duc, double Eel, double Atr,
	 ...
	 double etr = Str * (pow(DimSec, 3)) * (CA + 3 * CB) / (3 * Essec * Atr);
	 etru = duc * etry;
...
void superPosConfPress(Vector B, Vector semiLength,
	 Vector& frm, int secType, int dim, int i) {
...
void attSet(double epsc, double fc, double epsic, double fic,
...
ConfinedConcrete01::ConfinedConcrete01(int tag, Vector e, Vector s)
	 :UniaxialMaterial(tag, MAT_TAG_ConfinedConcrete01),
	 CminStrain(0.0), CendStrain(0.0),
	 Cstrain(0.0), Cstress(0.0), sigmac(s), eps(e)
{
}

ConfinedConcrete01::ConfinedConcrete01(int tag, int secType, int dim, Vector semiLength,
	 Vector phis, Vector S, Vector fyh, Vector Es0, Vector haRatio, Vector mueps,
	 Vector As, Vector Is, double rhos, double f, double stRatio, double Ec,
	 int epscuOption, double ecu, double epscuLimit, int nuOption, double nuc,
	 double phiLon, int concrType, int aggrType, double tol, int maxNumIter, double facToMpa
)
	 :UniaxialMaterial(tag, MAT_TAG_ConfinedConcrete01),
	 CminStrain(0.0), CendStrain(0.0),
	 Cstrain(0.0), Cstress(0.0)
{
	 ...
	 setupAttardSetunge(fpc, stRatio, Ec, aggrType, concrType,
		  epsc, epsic, fic, ft, fpl, alpha, Eti, Ec0);
	 ...
	 bglModel(semiLength, epscu, epscuOption, epscuLimit, nuOption, nuc, epsc, fc, epsic,
		  ...
		  phiLon, secType, dim, tol, maxNumIter, eps, sigmac, fpc, epsc0);

	 // Make all concrete parameters negative
	 epscu = -ecu;
	 fpcu = -sigmac[sigmac.Size() - 1];
...
void bglModel(Vector semiLength, double& epscu, int epscuOption, double epscuLimit,
	 int nuOption, double nuc, double epsc, double fc, double epsic, double fic, double ft, double fpl,
	 double alpha, double Eti, Vector phis, Vector As, Vector Is, Vector S, Vector fyh, Vector mueps,
	 Vector Es0, Vector haRatio, double phiLon, int secType, int dim, double tol, double maxNumIter,
	 Vector& eps, Vector& sigmac, double& fpc, double& epsc0) {
	 ...
	 */
	 ...
	 Vector sigmaTr(dim), epsTr(dim), A(dim), B(dim), frm(dim), Es(dim), ksl(dim);
	 ...
	 */
	 ...
	 for (j = 0; j < dim; j++) {
		  ...
		  k = confAlongCol(semiLength[j], phis[j], S[j], phiLon);
		  ksl(j) = k;
	 }
	 ...
	 int nEps = (epscu + 0.1 * dEpsZ) / dEpsZ + 1;
	 eps = Vector(nEps);
	 sigmac = Vector(nEps);
	 Vector& e(eps);
	 Vector& s(sigmac);
	 ...
	 Ec = Eti; // Initial concrete modulus
	 int totalIter = 0;
	 ...
	 for (i = 0; i < nEps; i++) {
		  ...
		  do { // Beginning of the iteration loop
			  ...
			  //---- Modulus of Poisson of concrete (by using the results obtained by Kupfer)
				...
				if (secType == 5) {  // S5 without wrapping  
					 ...
					 for (j = 0; j < dim; j++) {
						  trReinfModCircSec(fyh[j], mueps[j], Es0[j], As[j], semiLength[j],
								haRatio[j], B[j], epsTr[j], sigmaTr[j],
								Es[j]);
					 }
				}
				else { //Other Sections
					 for (j = 0; j < dim; j++) {
						  trReinfModSqSec(fyh[j], mueps[j], Es0[j], As[j], S[j], semiLength[j], haRatio[j],
								A[j], B[j], epsTr[j], sigmaTr[j], Es[j]);
					 }
				}
				// ----------------------------- Airy's Constants -----------------------
				if (secType == 5) { //S5 without wrapping
					 airyConSqSec(semiLength[0], Ec, nu, As[0], Is[0], Es[0],
						  S[0], e(i), A[0], B[0]);
					 B[1] = airyConCircSec(semiLength[1], Ec, nu, As[1], Es[1], S[1],
						  e(i));

					 if (dim == 3) //S5 with wrapping
						  airyConSqSec(semiLength[2], Ec, nu, As[2], Is[2], Es[2],
								S[2], e(i), A[2], B[2]);
				}
				else if (secType == 6) { //S6
					 for (j = 0; j < dim; j++) {
						  B[j] = airyConCircSec(semiLength[j], Ec, nu, As[j], Es[j],
								S[j], e(i));
					 }
				}
				else { //Other sections
					 for (j = 0; j < dim; j++) {
						  airyConSqSec(semiLength[j], Ec, nu, As[j], Is[j], Es[j],
								S[j], e(i), A[j], B[j]);
					 }
				}
				...
				//---- External transverse reinforcement
				if (secType == 5) {   // S5 without wrapping     
					 frm[0] = confPressSqSec(semiLength[0], B[0], ksl[0]);
					 frm[1] = confPressCircSec(S[1], B[1], ksl[1]);

					 if (dim == 3) //S5 with wrapping
						  frm[2] = confPressSqSec(semiLength[2], B[2], ksl[2]);
				}
				else if (secType == 6) { //S6
					 for (j = 0; j < dim; j++) {
						  frm[j] = confPressCircSec(S[j], B[j], ksl[j]);
					 }
				}
				else { //Other sections
					 for (j = 0; j < dim; j++) {
						  frm[j] = confPressSqSec(semiLength[j], B[j], ksl[j]);
					 }
				}

				// ---- Superposition of confining pressures
				// ---- Confining pressures are already multiplied by ksl (for wrapping ksl=1)
				superPosConfPress(B, semiLength, frm, secType, dim);

				// ---- (Secant) Modulus and stress of confined concrete by using Attard e Setunge      
				double EcInp = Ec; // Saving of concrete modulus used. Ec^k

				double EcOut;

				attSet(epsc, fc, epsic, fic, ft, fpl, alpha, Eti, e(i), frm[dim],
					 s[i], Ec, fcu, epscu, epscuOption, epscuLimit);
			  ...
			  //  }
				...
				EcOut = Ec; // Saving of concrete modulus obtained Ec*^k
		  ...
		  if (iter == maxNumIter) {
				opserr << "WARNING: Does not converge with specified maximum number of iterations" << endln;
		  }
		  totalIter += iter;
		  ...
		  if (s(i) > fcc) {
				fcc = s(i);  // Strength peak of confined concrete
				fpc = -fcc;
				epscc = e(i);    // Corresponding strain
				epsc0 = -epscc;
				if (epscuOption == 2)
...
// ------------Increment of axial strain for the next step---------------    
		  ...
		  e[i + 1] = e(i) + dEpsZ;  // Axial strain for the next step
...
void ConfinedConcrete01::envelope ()
{
	...
	if (Tstrain > 0.0 || Tstrain < epscu) {
		...
		for (int i = 0; i < (int)eps->Size(); i++) {
			 const double & e2 = (*eps)(i);
			 if (Tstrain > e2) {
				  const double& s2((*sigmac)(i)), s1((*sigmac)(i - 1)), e1((*eps)(i - 1));
				  Ttangent = (s2 - s1) / (e2 - e1);
				  Tstress = (Tstrain - e1) * Ttangent + s1;
				  break;
			 }
		}
}
...
OPS_ConfinedConcrete01Material()
{
	 if (numConfinedConcrete01Materials == 0) {
		  ...
		  opserr << "ConfinedConceret01 unaxial material - Initially written by M.D'Amato, University of Basilicata, Italy 2009\n";
		  opserr << "           Totally revised and re-written by S.A Jalali, Civil Soft Science, Iran, 2021\n";
	 }
  ...
  */
	 ...
	 */
	 ...
	 theMaterial = new ConfinedConcrete01(tag, secType, dim, Vector(semiLength), Vector(phis), Vector(S), Vector(fyh),
		  Vector(Es0), Vector(haRatio), Vector(mueps), Vector(As), Vector(Is), rhos, fpc, stRatio, Ec, epscuOption, epscu, epscuLimit,
		  nuOption, nuc, phiLon, concrType, aggrType, tol, maxNumIter, facToMPa
	 );
